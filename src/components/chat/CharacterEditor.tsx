...
export function CharacterEditor({ character, characters, onSave, onCancel, onDelete }: CharacterEditorProps) {
  ...
  return (
    <div className="fixed inset-0 bg-ios-bg z-[70] overflow-y-auto pb-20">
      <NavBar
      ...
      <div className="pt-28 px-4 space-y-6">
        ...
        <div className="bg-white rounded-xl p-4 space-y-4 shadow-sm">
          <div className="space-y-1.5">
            <Label>Nombre</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ej: Elena" />
          </div>

          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label>¿Es un grupo?</Label>
              <p className="text-xs text-ios-text-secondary">Permite juntar varias IAs</p>
            </div>
                ...
                {characters.filter(c => !c.isGroup).map(c => (
                  <button
                    ...
                    className={`flex items-center space-x-2 p-2 rounded-lg border text-sm transition-colors ${
                      memberIds.includes(c.id) ? 'bg-ios-blue/10 border-ios-blue' : 'bg-gray-50 border-gray-200'
                    }`}
                  >
          ...
          <div className="space-y-1.5">
            ...
            <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ej: Una bibliotecaria misteriosa" />
          </div>
